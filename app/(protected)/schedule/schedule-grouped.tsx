...
export default function ScheduleGrouped({ rows, today, companyCodeMap, showCompany }: Props) {
  ...
  async function exportExcel() {
    ...
    const data = selectedRows.map(r => ({
      'Scadenza': new Date(r.due_date + 'T00:00:00').toLocaleDateString('it-IT'),
      'Società': companyCodeMap?.[r.company_id] ?? '',
      'Fornitore': r.supplier_name ?? r.account_description ?? '',
      'Tipo documento': r.document_type ?? '',
      'N. documento': r.document_number ?? '',
      'Importo €': Math.abs(r.amount_cents) / 100,
      'Flusso': r.flow_type === 'out' ? 'Uscita' : 'Entrata',
      'Stato': STATUS_LABEL[r.status] ?? r.status,
      'Piano rientro': r.is_repayment_plan ? 'Sì' : '',
    }))
    ...
    XLSX.writeFile(wb, `pagamenti_${new Date().toISOString().split('T')[0]}.xlsx`)
  }

  const selectedTotal = rows.filter(r => selected.has(r.id)).reduce((s, r) => s + Math.abs(r.amount_cents), 0)
  ...
  return (
    <div className="relative">
              ...
              <tr className="border-b border-zinc-800 bg-zinc-900/80">
                <th className="px-3 py-2.5 w-8">
                  <input
                    ...
                    onChange={toggleAll}
                    className="rounded border-zinc-600 bg-zinc-800 text-indigo-500 focus:ring-indigo-500 focus:ring-offset-0 cursor-pointer"
                  />
                </th>
              ...
              {rows.map(item => {
                ...
                return (
                  <tr key={item.id} className={`border-b border-zinc-800/40 last:border-0 hover:bg-zinc-800/20 ${item.is_repayment_plan ? 'border-l-2 border-l-rose-500/50 bg-rose-500/[0.03]' : ''}`}>
                    ...
                    <td className={`px-3 py-2 text-right font-medium tabular-nums ${item.flow_type === 'out' && item.document_type !== 'NC' ? 'text-red-400' : 'text-emerald-400'}`}>
                      {item.flow_type === 'out' && item.document_type !== 'NC' ? '-' : '+'}{formatEur(Math.abs(item.amount_cents))}
                    </td>
                    <td className="px-3 py-2">
      ...
      {selected.size > 0 && (
        <div className="sticky bottom-4 mt-4 mx-auto w-fit flex items-center gap-4 px-5 py-3 bg-zinc-800 border border-zinc-700 rounded-xl shadow-2xl shadow-black/50 z-20">
          <span className="text-sm text-zinc-300">
            <span className="font-semibold text-zinc-100">{selected.size}</span> selezionat{selected.size === 1 ? 'o' : 'i'}
          </span>
          ...
          <button
            onClick={exportExcel}
            className="flex items-center gap-2 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-sm rounded-lg font-medium transition-colors"
          >
